...
class SS2D_xb(nn.Module):
    ...
    def forward(self, x: torch.Tensor,X_list = None, **kwargs):
        B, L, C = x.shape
        ...
        # x = x.permute(0, 3, 1, 2).contiguous()
        # x = self.act(x)
        # x = self.act(self.conv1d(x.permute(0,2,1))).permute(0,2,1) # (b, d, h, w)
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.self_attention = SS2D_xb(d_model=hidden_dim, dropout=attn_drop_rate, d_state=d_state, **kwargs)
